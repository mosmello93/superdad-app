    ...
    return (
        <div className="fixed inset-0 z-50 flex flex-col bg-[#F5F5F0] animate-in slide-in-from-bottom-full duration-500">
            ...
            <div className="bg-white px-6 pt-12 pb-6 shadow-sm flex items-center justify-between z-10">
                <div>
                    <h2 className="text-2xl font-bold text-stone-800 font-serif">Papierkram</h2>
                    <p className="text-stone-500 text-sm">Der Behörden-Guide für Dads</p>
                </div>
                    ...
                    {sortedTasks.map(task => {
                        ...
                        return (
                            <div key={task.id}
                                ...
                                className={`bg-white p-4 rounded-2xl border ${isDone ? 'border-emerald-100 opacity-60' : 'border-stone-100'} shadow-sm flex items-center gap-4 transition active:scale-95`}
                            >
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${isDone ? 'bg-emerald-100 text-emerald-600' : `bg-${asset.color}-50 text-${asset.color}-600`}`}>
                                    {isDone ? <CheckCircle size={24} /> : <Icon size={24} />}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-0.5">
                                        <h3 className={`font-bold text-stone-800 truncate ${isDone && 'line-through text-stone-400'}`}>{task.title}</h3>
                                        {task.urgency === 'high' && !isDone && <AlertCircle size={14} className="text-rose-500" />}
                                    </div>
                                    <p className="text-xs text-stone-500 truncate">{task.description}</p>
                                </div>
            ...
            {selectedTask && (
                <div className="absolute inset-0 z-50 bg-white animate-in slide-in-from-right duration-300 flex flex-col">
                    <div className="px-6 pt-12 pb-4 border-b border-stone-100 flex items-center gap-4 bg-white/90 backdrop-blur-md sticky top-0">
                        <button onClick={() => setSelectedTask(null)} className="p-2 -ml-2 text-stone-400 hover:text-stone-600">
                            <ChevronRight size={24} className="rotate-180" />
                        </button>
                        <h3 className="font-bold text-lg text-stone-800 flex-1 truncate">{selectedTask.title}</h3>
                    </div>

                    <div className="flex-1 overflow-y-auto p-6 pb-32">
                        {/* Meta Info */}
                        <div className="flex gap-2 mb-6 overscroll-x-auto">
                            <span className={`px-3 py-1 rounded-full text-xs font-bold bg-${BUREAUCRACY_ASSETS[selectedTask.category].color}-50 text-${BUREAUCRACY_ASSETS[selectedTask.category].color}-700`}>
                            ...
                            <div className="whitespace-pre-wrap text-stone-600 leading-relaxed">
                                {selectedTask.guide.split('\n').map((line, i) => {
                                    if (line.trim().startsWith('**')) {
                                        return <p key={i} className="font-bold text-stone-800 mt-4 mb-1">{line.replace(/\*\*/g, '')}</p>;
                                    }
                    ...
                    {/* Action Bar */}
                    <div className="p-6 border-t border-stone-100 bg-white safe-area-bottom">
                        <button
                            onClick={() => {
