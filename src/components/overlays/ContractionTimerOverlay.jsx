...
const ContractionTimerOverlay = ({ onClose }) => {
    ...
    const analyze = () => {
        if (contractions.length < 3) return { text: "Noch zu wenige Daten.", color: "text-stone-500" };

        // Avg distance of last 3
        const last3 = contractions.slice(0, 3);
        // Distance is stored on the item relative to the *previous* one in list (which is actually older).
        // Actually, distance logic above: `startTime - contractions[0].start`. 
        // contraction[0] is the one JUST finished. Its distance is relative to contraction[1].
        ...
        if (avgDistMin < 5 && avgDurSec > 45) {
            return { text: "Klinik anrufen / Losfahren? (5-1-1 Regel)", color: "text-red-600 font-bold" };
        }
        if (avgDistMin < 10) {
            return { text: "Es wird intensiver. Beobachten.", color: "text-amber-600 font-medium" };
        }
        return { text: "Alles ruhig. Entspann dich.", color: "text-emerald-600" };
    };
                    ...
                    <div className="overflow-y-auto space-y-3 pb-4 no-scrollbar">
                        {contractions.length === 0 ? (
                            ...
                            contractions.map((c, idx) => (
                                <div key={c.id} className="bg-white dark:bg-stone-900 p-4 rounded-2xl border border-stone-100 dark:border-stone-800 flex justify-between items-center shadow-sm">
